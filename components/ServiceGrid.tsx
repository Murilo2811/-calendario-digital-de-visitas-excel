...
export const ServiceGrid: React.FC<ServiceGridProps> = ({ services, technicians, clients, onUpdate, onDelete }) => {
  ...
  const EditableCell = ({ 
    ...
    list?: string
      ...
      return (
          <input 
            type={type}
            className={`w-full h-full bg-transparent border border-transparent rounded-md px-2 focus:bg-white focus:border-abb-red/50 focus:ring-1 focus:ring-abb-red/50 outline-none transition-all text-${align} ${className}`}
            value={tempValue}
        ...
        return (
          <tr key={service.id} className={`${rowBgClass} transition-colors group`}>
            ...
            <td className="p-0 border-b border-slate-100 relative h-10">
                <input 
                    type="date"
                    className={`w-full h-full bg-transparent text-center text-xs text-slate-700 cursor-pointer focus:bg-white focus:ring-1 focus:ring-abb-red/50 outline-none transition-colors ${!isStartDateValid ? 'bg-red-50 text-red-600 font-bold' : ''}`}
            ...
            <td className="p-0 border-b border-slate-100 relative h-10">
                 <input 
                    type="date"
                    className={`w-full h-full bg-transparent text-center text-xs text-slate-700 cursor-pointer focus:bg-white focus:ring-1 focus:ring-abb-red/50 outline-none transition-colors ${!isEndDateValid || isRangeInvalid ? 'bg-red-50 text-red-600 font-bold' : ''}`}
            ...
            <td className="p-0 border-b border-slate-100 relative h-10">
                 <input 
                    type="date"
                    className="w-full h-full bg-transparent text-center text-[10px] text-slate-500 cursor-pointer focus:bg-white focus:ring-1 focus:ring-abb-red/50 outline-none"
            ...
            <td className="p-0 h-10 border-b border-slate-100 text-center">
                 <input 
                    type="number"
                    list="period-options"
                    className="w-full h-full bg-transparent text-center text-slate-500 focus:bg-white focus:ring-1 focus:ring-abb-red/50 outline-none"
            ...
            <td className="px-2 py-1 border-b border-slate-100 text-center">
                <select 
                    ...
                    className={`w-full py-1.5 px-1 rounded-md text-[10px] font-bold uppercase text-center cursor-pointer focus:outline-none border-none ${statusClass}`}
                >
